/*
 * jQuery UI Datetimepicker  extended from jQuery UI Datepicker 1.7.2
 *
 *
 * Depends:
 *	ui.core.js
...
$.extend($.ui, { datetimepicker: { version: "1.7.2" } });//is datepicker's version

var PROP_NAME = 'datetimepicker';

/* Date and Time picker manager.
   Use the singleton instance of this class, $.datetimepicker, to interact with the datetime picker.
...
function Datetimepicker() {
	...
	this._mainDivId = 'ui-datetimepicker-div'; // The ID of the main datepicker division
	...
	this._defaults = { // Global defaults for all the date picker instances
		showTime: true, //true to show a time selecter
...
$.extend(Datetimepicker.prototype, {
	...
	_connectDatepicker: function(target, inst) {
		...
		if (showOn == 'button' || showOn == 'both') { // pop-up date picker when button clicked
			...
			inst.trigger.click(function() {
				if ($.datetimepicker._datepickerShowing && $.datetimepicker._lastInput == target)
			...
			switch (event.keyCode) {
				...
				case 13: var sel = $('td.' + $.datetimepicker._dayOverClass +
						...
						if (sel[0])
							...
							$.datetimepicker._hideDatepicker(null, $.datetimepicker._get(inst, 'duration'));
						return false; // don't submit the form
						break; // select the value on enter
				case 27: $.datetimepicker._hideDatepicker(null, $.datetimepicker._get(inst, 'duration'));
	...
	_doKeyPress: function(event) {
		...
		if ($.datetimepicker._get(inst, 'constrainInput')) {
			var datetimeFormat = $.datetimepicker._get(inst, 'dateFormat');
			if($.datetimepicker._get(inst,'showTime')){
				datetimeFormat += ' '+$.datetimepicker._get(inst, 'timeFormat');
			}
			var chars = $.datetimepicker._possibleChars(datetimeFormat);
	...
	_selectTime: function(id, select, period) {
		var target = $(id);
		var inst = this._getInst(target[0]);
		inst._selectingTime = false;
		inst['selected' + period] =
		inst['draw' + period] =
	...
	_selectDay: function(id, month, year, td) {
		...
		if(this._get(inst,'showTime')){
			hour = inst.selectedHour || inst.currentHour;
			minute = inst.selectedMinute || inst.currentMinute;
		}
		inst.selectedHour = inst.currentHour = hour;
		inst.selectedMinute = inst.currentMinute = minute;
	...
	_updateAlternate: function(inst) {
		...
		if (altField) { // update alternate field too
			var datetimeFormat = this._get(inst, 'dateFormat');
			if(this._get(inst,'showTime')){
				datetimeFormat += ' '+this._get(inst, 'timeFormat');
			}
			var altFormat = this._get(inst, 'altFormat') || datetimeFormat;
	...
	_setDateFromField: function(inst) {
		var dateFormat = this._get(inst, 'dateFormat') + (this._get(inst,'showTime')? ' '+this._get(inst, 'timeFormat') : '');
	...
	_getDate: function(inst) {
		var startDate = (!inst.currentYear || (inst.input && inst.input.val() == '') ? null :
			this._daylightSavingAdjust(this._get(inst,'showTime')?
	...
	_generateHTML: function(inst) {
		...
		var isRTL = this._get(inst, 'isRTL');
		var showButtonPanel = this._get(inst, 'showButtonPanel') || this._get(inst, 'showTime');
		...
		var controls = (!inst.inline ? '<button type="button" class="ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all" onclick="DP_jQuery.datetimepicker._hideDatepicker();">' + this._get(inst, 'closeText') + '</button>' : '');
		...
		var timeSelecter = '';
		if(this._get(inst, 'showTime')){
			timeSelecter = '<div class="ui-datetimepicker-time" style="text-align:right;"><select class="ui-datetimepicker-hour" style="border:0;" ' +
				'onchange="jQuery.datetimepicker._selectTime(\'#' + inst.id + '\', this, \'Hour\');" ' +
				'onclick="jQuery.datetimepicker._clickTime(\'#' + inst.id + '\');">';
			for (hour=0; hour < 24; hour++) {
				timeSelecter += '<option value="' + hour + '"' +
					(hour == drawHour ? ' selected="selected"' : '') +
					'>' + ((hour<10)?'0'+hour:hour) + '</option>';
			}
			timeSelecter += '</select>';
			timeSelecter += '&nbsp;:&nbsp;';
			timeSelecter += '<select class="ui-datetimepicker-minute" style="border:0;" ' +
				'onchange="jQuery.datetimepicker._selectTime(\'#' + inst.id + '\', this, \'Minute\');" ' +
				'onclick="jQuery.datetimepicker._clickTime(\'#' + inst.id + '\');">';
			for (minute=0; minute < 60; minute++) {
				timeSelecter += '<option value="' + minute + '"' +
					(minute == drawMinute ? ' selected="selected"' : '') +
					'>' + ((minute<10)?'0'+minute:minute) + '</option>';
			}
			timeSelecter += '</select></div>';
		}
		...
		html += timeSelecter + buttonPanel + ($.browser.msie && parseInt($.browser.version,10) < 7 && !inst.inline ?
	...
	_formatDate: function(inst, day, month, year, hour, minute) {
		...
		return this.formatDate(datetimeFormat, date, this._getFormatConfig(inst));
	}
});
...
jQuery(function($){
	$.datepicker.regional['zh-CN'] = {
		...
		dateFormat: 'yy-mm-dd', timeFormat: 'hh:ii', firstDay: 1,
		isRTL: false};
	$.datepicker.setDefaults($.datepicker.regional['zh-CN']);
	$.datetimepicker.setDefaults($.datepicker.regional['zh-CN']);
});
