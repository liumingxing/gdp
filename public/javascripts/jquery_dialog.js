...
var JqueryDialog = {
	...
	/// <summary>创建对话框(主调用函数)</summary>
	/// <param name="dialogTitle">对话框标题</param>
	/// <param name="iframeSrc">iframe嵌入页面地址</param>
	/// <param name="iframeWidth">iframe嵌入页面宽</param>
	/// <param name="iframeHeight">iframe嵌入页面高</param>
	/// <param name="isBottom">是否呈现底部区域(提交、取消按钮)</param>
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isBottom, isDrag){
		...
		_jd_dialog.css("left", (_left < 0 ? 0 : _left) + document.body.scrollLeft + "px");
		
		var _top = (document.body.clientHeight - (iframeHeight + JqueryDialog.cBorderSize * 2 + _px_top + _px_bottom + JqueryDialog.cShdowDown)) / 2;
	...
	/// <summary>提交完成（是否弹出对话框；是否关闭子窗口；是否刷新父页面）</summary>
	...
	/// <summary>提交完成（是否弹出对话框；关闭子窗口；父页面跳转）</summary>
	/// <param name="alertMsg">弹出提示内容，值为空不弹出</param>
	/// <param name="parentUrl">父页面跳转地址</param>
